...
async function sign(warp) {
    try {
        ...
        const curry_already_signed_pattern = /\[(.*?) -> 您\] 今日已經簽到過了，請明天再來/;
        const ddddo_already_signed_pattern = /\[(.*?) -> 您\] 今日已簽到，請明日再領取/;
        ...
        const ddddo_success_pattern = /\[(.*?) -> 您\] \[\d{2}:\d{2}:\d{2}\] 成功領取(.*?)/;
        ...
        try {
            const msg = await Promise.race([
                ...
                bot.awaitMessage(curry_already_signed_pattern),
                bot.awaitMessage(ddddo_success_pattern),
                new Promise((resolve) => setTimeout(() => resolve('Timeout'), 20000))
            ]);

            if (sign_in_success_pattern.test(msg) || ddddo_success_pattern.test(msg) || konjac_success_pattern.test(msg)) {
                ...
                // }
                continue_list.shift();
            } else if (konjac_already_signed_pattern.test(msg) || curry_already_signed_pattern.test(msg) || ddddo_already_signed_pattern.test(msg)) {
